...
- Ustawieniu Webpacka, plikiem wejściowym ma być `js/main.js`, wyjściowym `js/out.js`
...
* **Zaplecze** W folderze js znajdziesz wszystkie potrzebne pliki. Plikiem wejściowym jest plik ```main.js```. To on zostaje przetworzony przez Webpack, który na jego podstawie tworzy plik ```out.js```. Plik ```out.js``` jest dodany do pliku ```index.html```.
...
2. W pliku `main.js` importuj odpowiednią klasę oraz stwórz obiekt dla kalkulatora dziesiętnego.
